...
bucle_principal:
    ...
    MOVI2FP F4, R8           ;; Mover 8k+1 a F4 (entero)
    CVTI2F F4, F4            ;; Convertir 8k+1 a flotante
    LF F5, float4(R0)        ;; Cargar 4.0
    DIVF F6, F5, F4          ;; 4/(8k+1)
    NOP                      ;; Esperar resultado de DIVF (19 ciclos)
    NOP                      ;; Más NOPs para seguridad
    NOP
    SF calculoB(R3), F6      ;; Guardar 4/(8k+1)
    
    ;; 9. Calcular 2/(8k+4) (calculoC[k])
    MOVI2FP F7, R10          ;; Mover 8k+4 a F7 (entero)
    CVTI2F F7, F7            ;; Convertir 8k+4 a flotante
    LF F8, float2(R0)        ;; Cargar 2.0
    DIVF F9, F8, F7          ;; 2/(8k+4)
    NOP                      ;; Esperar resultado de DIVF
    NOP
    NOP
